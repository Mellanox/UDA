...
#tar $VERBOSE -C $LOGDIR --dereference --transform "s,^$STRIP,$JOB/`hostname`," --transform "s,.$ENDCHAR$,," --append -f $TARFILE  $LOGDIR/*$JOB* 2>&1 | grep -v "Removing leading" # takes the xml file, plus any snippet we created
...
tar $VERBOSE --dereference --transform "s,^$STRIP,$JOB/`hostname`," --append -f $TARFILE  $LOGDIR/*$TRACKER*.out 2> /dev/null || true
tar $VERBOSE --dereference --transform "s,^$STRIP,$JOB/`hostname`," --append -f $TARFILE  $LOGDIR/*$TRACKER*.out*[^$ENDCHAR] 2> /dev/null || true
