...
class UdaPluginTT extends UdaPlugin {    
	...
	public UdaPluginTT(TaskTracker taskTracker) {
		...
		launchCppSide(false, this.taskTracker.getJobConf(), null, ""); // false: this is TT => we should execute MOFSupplier
	}
	...
	public void notifyMapDone(String userName, String jobId, String mapId) {
		try {
			...
			Path fout = this.taskTracker.getLocalDirAllocator().getLocalPathToRead(
					TaskTracker.getIntermediateOutputDir(userName, jobId, mapId) 
					+ "/file.out", this.taskTracker.getJobConf());

			//parent path for the file.out.index file
			Path fidx = this.taskTracker.getLocalDirAllocator().getLocalPathToRead(
					TaskTracker.getIntermediateOutputDir(userName, jobId, mapId) 
					+ "/file.out.index", this.taskTracker.getJobConf());
