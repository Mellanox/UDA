...
if [[ $HADOOP_TYPE == "yarn" ]];then
	...
	status=1
	echo "$(basename $0): Starting DFS"
	while (($status != 0)) || (($curr_try < $max_tries));do		
		curr_try=$((curr_try + 1))
		echo "$(basename $0): attemp $curr_try"
		sbin/start-dfs.sh
		status=$?
	done
	if (($status != 0));then
		echo "$(basename $0): ERROR - failed to start dfs after $max_tries retries"
        exit 1
	fi
	
	status=1
	echo "$(basename $0): Starting YARN"
	while (($status != 0)) || (($curr_try < $max_tries));do		
		curr_try=$((curr_try + 1))
		echo "$(basename $0): attemp $curr_try"
		sbin/start-yarn.sh
	...
	if (($status != 0));then
		echo "$(basename $0): ERROR - failed to start yarn after $max_tries retries"
        exit 1
	fi
	hadoop_is_up=1
else
