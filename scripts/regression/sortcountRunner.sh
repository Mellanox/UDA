...
do				
	...
	do
		...
		export USER_CMD="$EXEC_JOB $HADOOP_EXAMPLES_JAR_EXP $CMD_PROGRAM $CMD_UDA_ENABLE $CMD_D_PARAMS $COMPRESSION_TEST_LEVEL $TEST_INPUT_DIR $TEST_OUTPUT_DIR"
		...
		if [[ $PROGRAM == "wordcount" ]];then
			...
			export USER_CMD_VANILLA="$EXEC_JOB $HADOOP_EXAMPLES_JAR_EXP $CMD_PROGRAM $COMPRESSION_TEST_LEVEL"
