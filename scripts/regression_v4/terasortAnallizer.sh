...
do
	...
	compression=$FAILURE_CODE
	if [[ -n $COMPRESSION ]];then
		compression="$SUCCESS_CODE &nbsp $COMPRESSION_TYPE"  
	fi
	...
	jobHeader="
			<tr bgcolor='$FILL_COLOR_1'>
				<td colspan='6'>$exec - Shuffle-Pro. $shuffP, Shuffle-Con. $shuffC, Compression. $compression, #Slaves: $SLAVES_COUNT, Data-set: ${tslDATA_SET}Gb, #Disks: $DISKS_COUNT, #Samples: ${tslSAMPLES} <br> \
	...
	do
		...
		testNum=$((testNum+1))	
		historyLog=$testDir/$JOB_HISTORY_FILE_NAME
	...
	execTable="<table border='1' cellpadding='6' cellspacing='6' width='100%'>
				$TERASORT_TESTS_NAMES
				$testsValues
			</table>"
	execTableDir=$execDir/${TERASORT_DATA_TABLE_FILE_NAME}_${currentDate}
	echo $execTable >> $execTableDir
	echo "$execTableDir" >> $REPORT_TABLES_MAPPER
	...
	statisticsTable="<table border='1' cellpadding='7' cellspacing='7' width='100%'>
						$jobHeader
						$TERASORT_STATISTICS_NAMES
						$statisticsValues
					</table>"
	statisticsTableDir=$execDir/${TERASORT_STAT_TABLE_FILE_NAME}_${currentDate}
	echo $statisticsTable >> $statisticsTableDir
	echo "$statisticsTableDir" >> $REPORT_TABLES_MAPPER
