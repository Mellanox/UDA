...
do
	...
	do
		...
		if (($exlWORDCOUNT_SUCCEED==1));then
			wordcountStatus=$SUCCESS_CODE
			succeededTests=$((succeededTests+1))
		else
			wordcountStatus=$FAILURE_CODE
		fi
...
allTablesDir=$allJobsDir/${WORDCOUNT_OUTCOME_TABLE_FILE_NAME}_${currentDate}
echo $allTables >> $allTablesDir
echo "$allTablesDir" >> $REPORT_TABLES_MAPPER

summaryDir=$allJobsDir/${WORDCOUNT_SUMMARY_TABLE_FILE_NAME}_${currentDate}
