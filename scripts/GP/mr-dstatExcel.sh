...
# finding the hosts that take part in this test
mapSlaves=$local_dir/mapSlaves.txt
reduceSlaves=$local_dir/reduceSlaves.txt
mapreduceSlaves=$local_dir/mapreduceSlaves.txt
hadoop fs -cat $OUTPUTDIR/_logs/history/*TeraSort | grep TRACKER_NAME | grep -e MAP | awk 'BEGIN{FS="\""};{ print $10}' | awk 'BEGIN{FS="_node"};{ print $2}' | awk 'BEGIN{FS="\\"};{ print $1}' | sort -u >> $local_dir/mapSlaves.txt
hadoop fs -cat $OUTPUTDIR/_logs/history/*TeraSort | grep TRACKER_NAME | grep -e REDUCE | awk 'BEGIN{FS="\""};{ print $10}' | awk 'BEGIN{FS="_node"};{ print $2}' | awk 'BEGIN{FS="\\"};{ print $1}' | sort -u >> $local_dir/reduceSlaves.txt
hadoop fs -cat $OUTPUTDIR/_logs/history/*TeraSort | grep TRACKER_NAME | grep -e MAP -e REDUCE | awk 'BEGIN{FS="\""};{ print $10}' | awk 'BEGIN{FS="_node"};{ print $2}' | awk 'BEGIN{FS="\\"};{ print $1}' | sort -u >> $local_dir/mapreduceSlaves.txt
mapSlavesCount=`cat $mapSlaves | grep -c ""`
reduceSlavesCount=`cat $reduceSlaves | grep -c ""`
mapreduceSlavesCount=`cat $mapreduceSlaves | grep -c ""`
...
#	echo $0: "cmd_status is: $cmd_status"
	...
	mv $collect_dir ${collect_dir}_ERROR
...
then
	...
	echo "Running TeraValidate!!"
	echo "$teravalidate"
	teravalidate="hadoop jar hadoop*-examples*.jar teravalidate $OUTPUTDIR $CURRENT_TERAVAL"
	eval $teravalidate
	valll="hadoop fs -ls $CURRENT_TERAVAL"
	...
	then
		...
		echo "hadoop fs -rmr /validate/out"
		hadoop fs -rmr $TERAVAL_DIR
	else
...
echo "
	...
	export exlMAP_SLAVES_COUNT=$mapSlavesCount
	export exlREDUCE_SLAVES_COUNT=$reduceSlavesCount
	export exlMAPREDUCE_SLAVES_COUNT=$mapreduceSlavesCount
" >> $logExports
...
echo "hadoop job -history $OUTPUTDIR | tee $local_dir/$readbleHistory.txt"
hadoop job -history $OUTPUTDIR | tee $local_dir/$readbleHistory.txt
echo "hadoop job -can $OUTPUTDIR | tee $local_dir/$readbleHistory.txt"
#sudo mkdir $TMP_DIR/historyAndConf
#hadoop job -cat $OUTPUTDIR/_logs/history/*TeraSort > /$TMP_DIR/historyAndConf/a.txt
fullConf=$TMP_DIR/configuration-Full.txt
#hadoop fs -cat $OUTPUTDIR/_logs/history/*conf.xml > $fullConf
...
mkdir -p $collect_dir/master-`hostname`/
...
mkdir -p $collect_dir/master-`hostname`/
#ssh $RES_SERVER chown -R $USER  $collect_dir/master-`hostname`/
cp -r $MY_HADOOP_HOME/logs/* $collect_dir/master-`hostname`/
cp -r $local_dir/* $collect_dir/
#cp $fullConf $collect_dir/
